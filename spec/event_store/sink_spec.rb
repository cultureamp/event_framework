...
RSpec.describe EventFramework::EventStore::Sink do
  ...
  context 'persisting a single event to the database' do
    ...
    let(:persisted_tuple) do
      persisted_tuples_for_aggregate(staged_event.aggregate_id).first
    end

    before do
      described_class.sink(staged_event)
    end
  ...
  describe 'optimistic locking' do
    context 'when the supplied aggregate_sequence has already been used' do
      ...
      it 'raises a concurrency error' do
        expect { described_class.sink(staged_event) }.to raise_error described_class::ConcurrencyError
      end

      it 'does not persist the event' do
        expect(persisted_tuples_for_aggregate(staged_event.aggregate_id).length).to eql 1
      end
    end
  end
